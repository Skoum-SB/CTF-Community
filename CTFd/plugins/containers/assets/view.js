...
function resetAlert() {
    ...
    alert.innerHTML = '<div class="spinner-border text-primary" role="status"><span class="visually-hidden"></span></div>';
...
function view_container_info(challenge_id) {
    ...
    .then(data => {
        ...
        if (data.status == "Challenge not started") {
            alert.innerHTML = "Pas de challenge en cours";
    ...
    .catch(error => {
        alert.innerHTML = "Erreur lors de la récupération des informations";
        alert.classList.add("alert-danger");
...
function container_stop(challenge_id) {
    ...
    .then(data => {
        ...
        if (data.error) {
            ...
            alert.innerHTML = "Fin du challenge.";
            toggleChallengeCreate();
            toggleChallengeUpdate();
        }
    })
    .catch(error => {
        alert.innerHTML = "Erreur pendant l'arrêt du challenge.";
        alert.classList.add("alert-danger");
        console.error("Fetch error:", error);
    })
    .finally(enableButtons);
